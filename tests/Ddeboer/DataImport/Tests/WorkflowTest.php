...
//            var_dump($item);
    ...
    public function testWorkflowWithObjects()
    {
        ...
        $converterStep = new ConverterStep([
            new CallbackItemConverter(function($item) {
                return array('name' => $item->name);
            })
        ]);

        $valueStep = new ValueConverterStep();
        $valueStep->add('name', new CallbackValueConverter(function($name) { return strrev($name); }));
    ...
    public function testItemConverterWhichReturnObjects()
    {
        ...
        $converterStep = new ConverterStep();
        $converterStep->add(new CallbackItemConverter(function($item) {
            return $item;
        }));
